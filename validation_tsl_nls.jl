...
#     error = sum(abs.(truevalue-pred).^2,dims=2);
#     if weights == 1
#         error = mean(sqrt.(error./sum(abs.(truevalue).^2, dims=2)))
#     else
#         error = sum(weights.*sqrt.(error./sum(abs.(truevalue).^2,dims=2)))/sum(weights)
#     end
#     return error
# end
...
function get_filename(param, TCRLB, N_TSL, SNR, crit, model)
    # return "../../matlab/TSL_opt_Hector/sampling_times/TSLs_$(param)_$(TCRLB)_N$(N_TSL)_SNR$(SNR)_$(crit)_customweights_$(model).mat"
    return "..//TSL_opt_Hector//sampling_times//TSLs_eggs_$(param)_$(TCRLB)_N$(N_TSL)_SNR$(SNR)_$(crit)_$(model).mat"
end
...
# idx = [optvars[i]=="mean" && time_rates[i]=="T1rho" ? i : 0 for i in 33:48];
# idx = idx[idx.>0]
# i = 17
# for i in idx
println("Job id $i")
