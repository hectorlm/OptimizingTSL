...
function gen_monoexp_data(N)
    ...
    # Create matrix where each column represents [A_i, T1ρ_i] for i = 1:N
    # Sample T1ρ values randomly for each A
...
if length(ARGS) == 0
    println("No command line arguments provided. Using default job id = 1")
    println("Usage: julia validation_tsl_nonopt_nls.jl <job_id>")
    println("Available job ids: 1 to $(length(mods))")
    i = 1
else
...
# Validate job id range
if i < 1 || i > length(mods)
    error("Job id $i is out of range. Available range: 1 to $(length(mods))")
end
