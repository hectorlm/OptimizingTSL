## Starting imports and function definitions
include("../../src/optimization/ann_fit.jl")
include("../../src/config_utils.jl")
include("../../src/validation_metrics.jl")
using .ValidationMetrics
using .ann_fit
using .ann_fit: def_model_stretch, getdata_stretch, gendata_stretch, Model_Args
using Flux
using Plots
using Statistics
using MAT
...
## Defining parameters, preparing structures and running trials

SNR, N_TSLs, mcrlb, crit, time_rate = get_param_xml("../../config/params.xml")
...
for inst in 1:length(param_args)
    ...
    print("\n")
    nepo = 5000+ 500*length(param_args[inst].TSLs)
    args = Model_Args(;n_epochs=nepo, TSL=param_args[inst].TSLs, SNR=param_args[inst].SNR, debug=true, lw=24, batchsize=2000)
    R2t = zeros(2, trials);
    ...
    for trial in 1:trials
        print("\r Trial $trial")
        model = def_model_stretch(args.N_TSL, args.lw);
        dataloaders = getdata_stretch(args);
        # @time model = train(model, dataloaders, args)
        model = train(model, dataloaders, args)

        Flux.testmode!(model)
        x,y = gendata_stretch(args, 5000)
        predy = model(x)
