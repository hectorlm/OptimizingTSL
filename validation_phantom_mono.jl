...
@time @threads for i in findall(>(0),fmask)
    p̂[:, i] = nls_trcg(fimgs[i, :], tsl, [1.0+0im; 45], bds, "monoexp", "T1rho")[1];
end

pAmap = reshape((p̂[1,:]), size(data["mask"]));
pTmap = reshape(abs.(p̂[2,:]), size(data["mask"]));

gui = imshow_gui((256,256), (2,3))
canvases = gui["canvas"];
